...
describe :array_pack_8bit, shared: true do
  ...
  it "calls #to_int to convert the pack argument to an Integer" do
    ...
    expect([obj].pack(pack_format)).to eq("\x05")
  end

  it "encodes the number of array elements specified by the count modifier" do
    expect([ [[1, 2, 3], pack_format(3), "\x01\x02\x03"],
  ...
  it "encodes all remaining elements when passed the '*' modifier" do
    expect([1, 2, 3, 4, 5].pack(pack_format('*'))).to eq("\x01\x02\x03\x04\x05")
  end

  it "ignores NULL bytes between directives" do
    expect([1, 2, 3].pack(pack_format("\000", 2))).to eq("\x01\x02")
  end

  it "ignores spaces between directives" do
    expect([1, 2, 3].pack(pack_format(' ', 2))).to eq("\x01\x02")
  end
end
