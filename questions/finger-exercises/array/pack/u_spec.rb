...
describe "Array#pack with format 'u'" do
  ...
  it "emits a newline after complete groups of count / 3 input characters when passed a count modifier" do
    expect(["abcdefg"].pack("u3")).to eq("#86)C\n#9&5F\n!9P``\n")
  end
  ...
  it "encodes all ascii characters" do
    expect([ [["\x00\x01\x02\x03\x04\x05\x06"],          "'``$\"`P0%!@``\n"],
      [["\a\b\t\n\v\f\r"],                        "'!P@)\"@L,#0``\n"],
  ...
  it "raises a TypeError if passed nil" do
    expect { [nil].pack("u") }.to raise_error(TypeError)
  end

  it "raises a TypeError if passed an Integer" do
    expect { [0].pack("u") }.to raise_error(TypeError)
    expect { [bignum_value].pack("u") }.to raise_error(TypeError)
  end

  it "sets the output string to US-ASCII encoding" do
    expect(["abcd"].pack("u").encoding).to eq(Encoding::US_ASCII)
  end
end
