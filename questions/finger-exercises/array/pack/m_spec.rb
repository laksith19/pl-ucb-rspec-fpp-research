...
describe "Array#pack with format 'M'" do
  ...
  it "encodes an '=' character in hex format" do
    expect(["="].pack("M")).to eq("=3D=\n")
  end

  it "encodes an embedded space directly" do
    expect(["a b"].pack("M")).to eq("a b=\n")
  end
  ...
  it "encodes 0..31 except tab and newline in hex format" do
    expect([ [["\x00\x01\x02\x03\x04\x05\x06"],  "=00=01=02=03=04=05=06=\n"],
  ...
  it "encodes 127..255 in hex format" do
    expect([ [["\x7f\x80\x81\x82\x83\x84\x85\x86"], "=7F=80=81=82=83=84=85=86=\n"],
  ...
  it "encodes a recursive array" do
    ...
    expect(empty.pack('M')).to be_an_instance_of(String)

    array = ArraySpecs.recursive_array
    expect(array.pack('M')).to eq("1=\n")
  end

  it "calls #to_s to convert an object to a String" do
    obj = double("pack M string")
    expect(obj).to receive(:to_s).and_return("packing")

    expect([obj].pack("M")).to eq("packing=\n")
  end

  it "converts the object to a String representation if #to_s does not return a String" do
    obj = double("pack M non-string")
    expect(obj).to receive(:to_s).and_return(2)

    expect([obj].pack("M")).to be_an_instance_of(String)
  end

  it "encodes a Symbol as a String" do
    expect([:symbol].pack("M")).to eq("symbol=\n")
  end
  ...
  it "encodes a Float as a String" do
    expect([1.0].pack("M")).to eq("1.0=\n")
  end

  it "converts Floats to the minimum unique representation" do
    expect([1.0 / 3.0].pack("M")).to eq("0.3333333333333333=\n")
  end

  it "sets the output string to US-ASCII encoding" do
    expect(["abcd"].pack("M").encoding).to eq(Encoding::US_ASCII)
  end
end
...
describe "Array#pack with format 'm'" do
  ...
  it "appends a newline to the end of the encoded string" do
    expect(["a"].pack("m")).to eq("YQ==\n")
  end

  it "encodes one element per directive" do
    expect(["abc", "DEF"].pack("mm")).to eq("YWJj\nREVG\n")
  end
  ...
  it "emits a newline after complete groups of count / 3 input characters when passed a count modifier" do
    expect(["abcdefg"].pack("m3")).to eq("YWJj\nZGVm\nZw==\n")
  end
  ...
  it "encodes all ascii characters" do
    expect([ [["\x00\x01\x02\x03\x04\x05\x06"],          "AAECAwQFBg==\n"],
      [["\a\b\t\n\v\f\r"],                        "BwgJCgsMDQ==\n"],
  ...
  it "calls #to_str to convert an object to a String" do
    ...
    expect([obj].pack("m")).to eq("YWJj\n")
  end

  it "raises a TypeError if #to_str does not return a String" do
    obj = double("pack m non-string")
    expect { [obj].pack("m") }.to raise_error(TypeError)
  end

  it "raises a TypeError if passed nil" do
    expect { [nil].pack("m") }.to raise_error(TypeError)
  end

  it "raises a TypeError if passed an Integer" do
    expect { [0].pack("m") }.to raise_error(TypeError)
    expect { [bignum_value].pack("m") }.to raise_error(TypeError)
  end
