...
describe :array_pack_16bit_le, shared: true do
  it "encodes the least significant 16 bits of a positive number" do
    expect([ [[0x0000_0021], "\x21\x00"],
      [[0x0000_4321], "\x21\x43"],
      [[0x0065_4321], "\x21\x43"],
      [[0x7865_4321], "\x21\x43"]
    ]).to be_computed_by(:pack, pack_format())
  end

  it "encodes the least significant 16 bits of a negative number" do
    expect([ [[-0x0000_0021], "\xdf\xff"],
      [[-0x0000_4321], "\xdf\xbc"],
      [[-0x0065_4321], "\xdf\xbc"],
      [[-0x7865_4321], "\xdf\xbc"]
    ]).to be_computed_by(:pack, pack_format())
  ...
  it "calls #to_int to convert the pack argument to an Integer" do
    obj = double('to_int')
    expect(obj).to receive(:to_int).and_return(0x1234_5678)
    expect([obj].pack(pack_format())).to eq("\x78\x56")
  end
...
describe :array_pack_16bit_be, shared: true do
  ...
  it "calls #to_int to convert the pack argument to an Integer" do
    obj = double('to_int')
    expect(obj).to receive(:to_int).and_return(0x1234_5678)
    expect([obj].pack(pack_format())).to eq("\x56\x78")
  end

  it "encodes the number of array elements specified by the count modifier" do
    str = [0x1243_6578, 0xdef0_abcd, 0x7865_4321].pack(pack_format(2))
    expect(str).to eq("\x65\x78\xab\xcd")
  end

  it "encodes all remaining elements when passed the '*' modifier" do
    str = [0x1243_6578, 0xdef0_abcd, 0x7865_4321].pack(pack_format('*'))
    expect(str).to eq("\x65\x78\xab\xcd\x43\x21")
  end
...
describe :array_pack_32bit_le, shared: true do
  it "encodes the least significant 32 bits of a positive number" do
    expect([ [[0x0000_0021], "\x21\x00\x00\x00"],
  ...
  it "encodes the least significant 32 bits of a negative number" do
    expect([ [[-0x0000_0021], "\xdf\xff\xff\xff"],
  ...
  it "encodes a Float truncated as an Integer" do
    expect([ [[2019902241.2],  "\x21\x43\x65\x78"],
  ...
  it "calls #to_int to convert the pack argument to an Integer" do
    obj = double('to_int')
    expect(obj).to receive(:to_int).and_return(0x1234_5678)
    expect([obj].pack(pack_format())).to eq("\x78\x56\x34\x12")
  end

  it "encodes the number of array elements specified by the count modifier" do
    str = [0x1243_6578, 0xdef0_abcd, 0x7865_4321].pack(pack_format(2))
    expect(str).to eq("\x78\x65\x43\x12\xcd\xab\xf0\xde")
  end

  it "encodes all remaining elements when passed the '*' modifier" do
    str = [0x1243_6578, 0xdef0_abcd, 0x7865_4321].pack(pack_format('*'))
    expect(str).to eq("\x78\x65\x43\x12\xcd\xab\xf0\xde\x21\x43\x65\x78")
  end
...
describe :array_pack_32bit_be, shared: true do
  it "encodes the least significant 32 bits of a positive number" do
    expect([ [[0x0000_0021], "\x00\x00\x00\x21"],
  ...
  it "encodes the least significant 32 bits of a negative number" do
    expect([ [[-0x0000_0021], "\xff\xff\xff\xdf"],
  ...
  it "encodes a Float truncated as an Integer" do
    expect([ [[2019902241.2],  "\x78\x65\x43\x21"],
  ...
  it "calls #to_int to convert the pack argument to an Integer" do
    obj = double('to_int')
    expect(obj).to receive(:to_int).and_return(0x1234_5678)
    expect([obj].pack(pack_format())).to eq("\x12\x34\x56\x78")
  end

  it "encodes the number of array elements specified by the count modifier" do
    str = [0x1243_6578, 0xdef0_abcd, 0x7865_4321].pack(pack_format(2))
    expect(str).to eq("\x12\x43\x65\x78\xde\xf0\xab\xcd")
  end

  it "encodes all remaining elements when passed the '*' modifier" do
    str = [0x1243_6578, 0xdef0_abcd, 0x7865_4321].pack(pack_format('*'))
    expect(str).to eq("\x12\x43\x65\x78\xde\xf0\xab\xcd\x78\x65\x43\x21")
  end
...
describe :array_pack_32bit_le_platform, shared: true do
  it "encodes the least significant 32 bits of a number" do
    expect([ [[0x7865_4321],  "\x21\x43\x65\x78"],
      [[-0x7865_4321], "\xdf\xbc\x9a\x87"]
    ]).to be_computed_by(:pack, pack_format())
  end
  ...
  platform_is wordsize: 64 do
    it "encodes the least significant 32 bits of a number that is greater than 32 bits" do
      expect([ [[0xff_7865_4321],  "\x21\x43\x65\x78"],
...
describe :array_pack_32bit_be_platform, shared: true do
  it "encodes the least significant 32 bits of a number" do
    expect([ [[0x7865_4321],  "\x78\x65\x43\x21"],
      [[-0x7865_4321], "\x87\x9a\xbc\xdf"]
    ]).to be_computed_by(:pack, pack_format())
  end
  ...
  platform_is wordsize: 64 do
    it "encodes the least significant 32 bits of a number that is greater than 32 bits" do
      expect([ [[0xff_7865_4321],  "\x78\x65\x43\x21"],
...
describe :array_pack_64bit_le, shared: true do
  it "encodes the least significant 64 bits of a positive number" do
    expect([ [[0x0000_0000_0000_0021], "\x21\x00\x00\x00\x00\x00\x00\x00"],
  ...
  it "encodes the least significant 64 bits of a negative number" do
    expect([ [[-0x0000_0000_0000_0021], "\xdf\xff\xff\xff\xff\xff\xff\xff"],
  ...
  it "encodes a Float truncated as an Integer" do
    expect([ [[9.14138647331322368e+18],  "\x00\x44\x65\x78\x90\xba\xdc\x7e"],
  ...
  it "calls #to_int to convert the pack argument to an Integer" do
    obj = double('to_int')
    expect(obj).to receive(:to_int).and_return(0x1234_5678_90ab_cdef)
    expect([obj].pack(pack_format())).to eq("\xef\xcd\xab\x90\x78\x56\x34\x12")
  end
  ...
  it "encodes the number of array elements specified by the count modifier" do
    ...
    expect(str).to eq("\xef\xcd\xab\x90\x78\x56\x34\x12\x56\x78\x12\x34\xcd\xab\xf0\xde")
  end

  it "encodes all remaining elements when passed the '*' modifier" do
    str = [0xdef0_abcd_3412_7856, 0x7865_4321_dcba_def0].pack(pack_format('*'))
    expect(str).to eq("\x56\x78\x12\x34\xcd\xab\xf0\xde\xf0\xde\xba\xdc\x21\x43\x65\x78")
  end
...
describe :array_pack_64bit_be, shared: true do
  it "encodes the least significant 64 bits of a positive number" do
    expect([ [[0x0000_0000_0000_0021], "\x00\x00\x00\x00\x00\x00\x00\x21"],
  ...
  it "encodes the least significant 64 bits of a negative number" do
    expect([ [[-0x0000_0000_0000_0021], "\xff\xff\xff\xff\xff\xff\xff\xdf"],
  ...
  it "encodes a Float truncated as an Integer" do
    expect([ [[9.14138647331322368e+18],  "\x7e\xdc\xba\x90\x78\x65\x44\x00"],
  ...
  it "calls #to_int to convert the pack argument to an Integer" do
    obj = double('to_int')
    expect(obj).to receive(:to_int).and_return(0x1234_5678_90ab_cdef)
    expect([obj].pack(pack_format())).to eq("\x12\x34\x56\x78\x90\xab\xcd\xef")
  end
  ...
  it "encodes the number of array elements specified by the count modifier" do
    str = [0x1234_5678_90ab_cdef,
           0xdef0_abcd_3412_7856,
           0x7865_4321_dcba_def0].pack(pack_format(2))
    expect(str).to eq("\x12\x34\x56\x78\x90\xab\xcd\xef\xde\xf0\xab\xcd\x34\x12\x78\x56")
  end

  it "encodes all remaining elements when passed the '*' modifier" do
    str = [0xdef0_abcd_3412_7856, 0x7865_4321_dcba_def0].pack(pack_format('*'))
    expect(str).to eq("\xde\xf0\xab\xcd\x34\x12\x78\x56\x78\x65\x43\x21\xdc\xba\xde\xf0")
  end
